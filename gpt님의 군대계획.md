...
그런데 휴가가 많기 때문에 실제로는 **8~12월을 핵심 공부기간**, **1월을 복습/정리 기간**으로 보는 게 좋다.
...
이때는 책을 보는 것보다 **질문에 답하는 것**을 한다.

예:

> `vector`와 `list`의 차이는?

> `map`과 `unordered_map`의 차이는?

> `unique_ptr`와 `shared_ptr`의 차이는?
...
여기부터 네가 이미 읽은 **네트워크 하향식 접근**을 다시 활용한다.
...
1. RAII란?
2. unique_ptr와 shared_ptr 차이는?
3. move semantics란?
4. virtual이 필요한 이유는?
5. vector와 list 차이는?
6. map과 unordered_map 차이는?
...
12. IP와 Port 차이는?
13. TCP와 UDP 차이는?
14. TCP 3-way handshake란?
15. TCP가 byte stream이라는 것은 무슨 뜻인가?
16. socket이란?
17. listen/accept는 무엇을 하는가?
...
25. Room이란?
26. Game Loop란?
27. 서버가 클라이언트를 믿으면 안 되는 이유는?
28. 게임 서버에서 TCP를 사용할 때 Packet 구조가 왜 필요한가?
...
즉, **군대 6개월을 끝내고 전역하자마자 "뭐부터 하지?"가 아니라 바로 Socket Programming부터 시작할 수 있는 상태**를 만드는 것이다.
...
지금까지 네 공부가 `열혈 C++ 1회독 → 네트워크 1회독 → OSTEP 1회독 → 바킹독 1회독` 식으로 흘러갔다면, 이번 6개월은 그걸 **한 줄의 지식 체계로 연결하는 기간**으로 쓰는 게 가장 가치 있다.
